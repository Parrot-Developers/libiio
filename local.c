...
static ssize_t write_all(const void *dst, size_t len, int fd)
{
	uintptr_t ptr = (uintptr_t) dst;
	ssize_t writtensize;
	int ret;
	...
	writtensize = (ssize_t)(ptr - (uintptr_t) dst);
...
static int local_get_poll_fd(const struct iio_device *dev)
{
	return dev->pdata->fd;
}
